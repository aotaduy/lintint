...
CLI.prototype.showHelp = function() {
    ...
    console.info('--config=global\t: <default> Use <lintint>/config/ configuration files');
    console.info('--config=local\t: Use project configuration files: .jscsrc .eslintrc .jshintrc ...');
...
CLI.prototype.setConfigurationResolver = function(aString, dirs) {
    ...
    if (aString === 'local') {
        ConfigurationResolver.initializeLocal(localDir);
    }
};
